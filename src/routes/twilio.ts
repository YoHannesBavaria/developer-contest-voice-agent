...
function buildGatherTwiml(text: string, callId: string, publicBaseUrl?: string): string {
  ...
  return [
    ...
    "<Response>",
    `  <Gather input="speech dtmf" language="de-DE" speechModel="phone_call" speechTimeout="auto" timeout="10" actionOnEmptyResult="true" method="POST" action="${xmlEscape(actionUrl)}">`,
