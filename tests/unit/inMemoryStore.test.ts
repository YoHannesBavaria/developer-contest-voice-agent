...
describe("InMemoryCallStore latency KPIs", () => {
  it("calculates average, p95 and under-1500ms rate", () => {
    const store = new InMemoryCallStore();

    store.addVoiceLatency("call-1", 900);
    store.addVoiceLatency("call-1", 1200);
    store.addVoiceLatency("call-1", 2000);
    store.addVoiceLatency("call-2", 800);
